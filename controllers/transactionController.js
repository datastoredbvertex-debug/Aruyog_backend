...
const getAllTransactions = asyncHandler(async (req, res) => {
      try {
        ...
        const transactions = await Transaction.find()
          ...
          .populate({
            path: "user_id",
            select: "full_name email", // Specify fields to populate from user
          })
          .populate({
            path: "teacher_id",
        ...
        // Count total transactions for calculating total pages
        const totalTransactions = await Transaction.countDocuments();

        // Send paginated transaction data in the response
        res.json({
          Transactions: transactions,
          total_rows: totalTransactions,
          current_page: page,
          total_pages: Math.ceil(totalTransactions / limit), // Calculate total pages
        });
      } catch (error) {
