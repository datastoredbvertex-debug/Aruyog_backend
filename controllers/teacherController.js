...
const addCourse = asyncHandler(async (req, res, next) => {
  ...
  upload.single("course_image")(req, res, async (err) => {
    ...
    try {
      ...
      const filtered = coursess.filter((co) => {
        ...
        console.log(formattedDate, todayy, todayy <= formattedDate);
        return todayy <= formattedDate;
      });
      ...
      //   teacher_id,
      //   $or: [
      //     { endDate: null }, // Include courses with no endDate
      //     { endDate: { $gte: today.format("DD/MM/YYYY") } }, // Include only non-expired courses
      //   ],
      // });
...
const getTodayCourse = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const courses = await Course.find({
      ...
      deleted_at: null,
      // Ensure both `userIds` and `cb_id` are not null and not empty
      userIds: { $ne: null, $ne: [] },
      cb_id: { $ne: null, $ne: [] },
      // Check `startDate` and `endDate` conditions
      $or: [
        { startDate: { $lte: formattedCurrentDate } }, // `startDate` is before or equal to current date
        { startDate: null }, // or `startDate` is null
      ],
      $or: [
        { endDate: { $gte: formattedCurrentDate } }, // `endDate` is after or equal to current date
        { endDate: null }, // or `endDate` is null
      ],
    })
...
const teacherUnavailabilityDate = async (req, res) => {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(userId, { $push: { teacherUnavailabilityDates: { $each: unavailabilityWithTimestamps } } }, { new: true, runValidators: true });
...
const cleanOldUnavailabilityDates = async () => {
  ...
  try {
    ...
    const result = await User.updateMany({ "teacherUnavailabilityDates.addedAt": { $lt: threeMonthsAgo } }, { $pull: { teacherUnavailabilityDates: { addedAt: { $lt: threeMonthsAgo } } } });
...
const updateTeacherDocument = async (req, res) => {
  ...
  upload.fields([
    { name: "teacherDocument", maxCount: 5 }, // Allow up to 5 files for teacherDocument
    { name: "addharImages", maxCount: 2 }, // Allow up to 5 files for addharImages
  ])(req, res, async (err) => {
    if (err) {
    ...
    try {
      // Process uploaded teacherDocument files
      const teacherDocuments = req.files.teacherDocument ? req.files.teacherDocument.map((file) => ({ image: `${req.uploadPath}/${file.filename}` })) : [];

      // Process uploaded addharImages files
      const addharImages = req.files.addharImages ? req.files.addharImages.map((file) => ({ addharimage: `${req.uploadPath}/${file.filename}` })) : [];
      ...
      const updatedUser = await User.findByIdAndUpdate(
        userID,
        {
          $set: {
            ...
            addharImages, // Store array of objects with addharimage field
          },
        },
...
const updateAadhaarImages = async (req, res) => {
      const userID = req.headers.userID;
      req.uploadPath = "uploads/teacherDocument";

      upload.fields([{ name: "addharImages", maxCount: 2 }])(req, res, async (err) => {
        if (err) {
          return res.status(400).json({ message: "File upload error", error: err });
        }
        ...
        if (!req.files.addharImages) {
          return res.status(400).json({ message: "Aadhaar images are required" });
        }

        try {
          const addharImages = req.files.addharImages.map((file) => ({
            addharimage: `${req.uploadPath}/${file.filename}`,
          }));
          ...
          res.status(200).json({ message: "Aadhaar images updated successfully", data: updatedUser });
        } catch (error) {
          res.status(500).json({ message: "Internal server error", error });
        }
      });
...
const updateTeacherDocuments = async (req, res) => {
      const userID = req.headers.userID;
      req.uploadPath = "uploads/teacherDocument";

      upload.fields([{ name: "teacherDocument", maxCount: 5 }])(req, res, async (err) => {
        if (err) {
          return res.status(400).json({ message: "File upload error", error: err });
        }
        ...
        if (!req.files.teacherDocument) {
          return res.status(400).json({ message: "Teacher documents are required" });
        }

        try {
          const teacherDocuments = req.files.teacherDocument.map((file) => ({
            image: `${req.uploadPath}/${file.filename}`,
          }));

          const updatedUser = await User.findByIdAndUpdate(
            userID,
            { $set: { teacherDocuments } },
            { new: true, runValidators: true }
          );
          ...
          res.status(200).json({ message: "Teacher documents updated successfully", data: updatedUser });
        } catch (error) {
          res.status(500).json({ message: "Internal server error", error });
        }
      });
...
const updateVerifyStatus = async (req, res) => {
      ...
      if (!verifyStatus) {
        return res.status(400).json({ message: "Verify status is required" });
      }
...
const updateTeacherStatus = async (req, res) => {
  const { teacher_id, verifyStatus, type, reject_msg } = req.body;
  ...
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(teacher_id, { $set: { verifyStatus } }, { new: true, runValidators: true });
    ...
    if (verifyStatus === "approved") {
      ...
      console.log("payid", paymentTypes);
      ...
      await User.findByIdAndUpdate(teacher_id, { $set: { reject_msg } }, { new: true, runValidators: true });
    }
...
const checkExpiredCoursesAndNotify = async () => {
      try {
        // Get the current date
        const currentDate = new Date();

        // Find courses created more than 21 days ago
        const expiredCourses = await Course.find({
          g: { $lt: new Date(currentDate - 21 * 24 * 60 * 60 * 1000) }, // 21 days ago
        });

        for (let course of expiredCourses) {
          const teacher = await User.findById(course.teacher_id);

          if (teacher && teacher.firebase_token) {
            const registrationToken = teacher.firebase_token;
            const title = `Course Expired`;
            const body = `Your course has expired.`;
        ...
        console.error("Error checking expired courses:", error.message);
      }
    };



const sendNotificationsForAllCourses = async () => {
  try {
    const currentTime = moment(); // Get the current time as a moment object

    // Find courses based on today's date in startDate
    const coursesToNotify = await Course.find({
      startDate: currentTime.format("YYYY/MM/DD"),
    });

    for (const course of coursesToNotify) {
      const courseStartTime = moment(course.startTime, "h:mm A"); // Parse course start time in the "h:mm A" format

      // Adjust the course start time to today’s date for comparison
      courseStartTime.set("year", currentTime.year());
      courseStartTime.set("month", currentTime.month());
      courseStartTime.set("date", currentTime.date());

      // Compare current time with the course start time
      if (currentTime.isSame(courseStartTime, "minute")) {
        // Check if the current time matches the course start time (to the minute)

        // Notify the teacher
        const teacher = await User.findById(course.teacher_id);
        if (teacher && teacher.firebase_token) {
          const title = "Course Start Notification";
          const body = `Your course has started at ${courseStartTime.format("h:mm A")}.`;

          const notificationResult = await sendFCMNotification(teacher.firebase_token, title, body);
          if (notificationResult.success) {
            console.log("Notification sent successfully to teacher for course:", course._id);
          } else {
            console.error("Failed to send notification to teacher for course:", course._id, notificationResult.error);
          }
        }

        // Notify the users in the course
        for (const userId of course.userIds) {
          const user = await User.findById(userId);
          if (user && user.firebase_token) {
            const studentTitle = "Course Start Notification";
            const studentBody = `Your course has started at ${courseStartTime.format("h:mm A")}.`;
            const notificationResult = await sendFCMNotification(user.firebase_token, studentTitle, studentBody);
            if (notificationResult.success) {
              console.log("Notification sent successfully to user:", userId, "for course:", course._id);
            } else {
              console.error("Failed to send notification to user:", userId, "for course:", course._id, notificationResult.error);
            }
          }
        }

        for (const userId of course.askDemoids) {
          const user = await User.findById(userId);
          if (user && user.firebase_token) {
            const studentTitle = "Course Start Notification";
            const studentBody = `Your course has started at ${courseStartTime.format("h:mm A")}.`;
    ...
    console.error("Error sending course start notifications:", error.message);
  }
};
...
module.exports = {
  ...
  updateCourseDates,
  getTeacherProfileDataByTeacherId,
  CourseActiveStatus,
  autoDeactivateCourses,
  teacherUnavailabilityDate,
  updateTeacherDocument,
  getteacherUnavailabilityDateById,
  notifyTeachersAboutEndingCourses,
  updateTeacherStatus,
  updateCourse,
  cleanOldUnavailabilityDates,
  sendNotificationsForAllCourses,
  updateAadhaarImages,
