...
//     port: null,
//     path: `/api/v5/otp?template_id=${process.env.TEMPLATE_ID}&mobile=91${mobile}&authkey=${process.env.MSG91_API_KEY}&realTimeResponse=0`,
//     headers: {
...
function sendOTP(name, mobile, otp) {
  console.log("📩 Sending OTP to:", name, mobile);

  if (!process.env.TEMPLATE_ID || !process.env.MSG91_API_KEY) {
    console.error("❌ Missing MSG91 API Key or Template ID in environment variables.");
    return;
  }
  ...
  const requestBody = JSON.stringify({
    template_id: process.env.TEMPLATE_ID, // Your MSG91 Flow Template ID
    short_url: "0", // Use "1" to enable short URLs, "0" to disable
    realTimeResponse: "1",
    recipients: [
      {
        mobiles: `91${mobile}`,
        var: otp, // Ensure your MSG91 Flow template uses {VAR1} for OTP
      },
    ],
  });
...
// Example Usage:
// sendOTP("John Doe", 7049866959, "5555");
...
const verifyOtp = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user.otp_verified) {
      throw new ErrorHandler("User is already OTP verified.", 400); // This is a 400 error
    }
    ...
    // Now, after OTP verification, send the values to createConnectyCubeUser
    ...
    // Handle Error properly based on its status code
    if (error instanceof ErrorHandler) {
      console.error("ErrorHandler:", error.message);
      // Return the appropriate status code based on the error
    ...
    // If it's an unhandled error, return a 500 status
    res.status(500).json({
      message: error.message || "Internal Server Error",
      status: false,
    });
...
const bank_Detail_create = asyncHandler(async (req, res) => {
  ...
  const userId = req.headers.userID; // Assuming user authentication middleware provides this
  ...
  try {
    ...
    if (bankDetails) {
      // Update the existing document with provided fields
      const updatedFields = {
        ...(bankName && { bankName }),
...
//     const users = await User.find({ role: "student" }).sort({ updatedAt: -1 }).skip(skip).limit(Number(limit));
...
const getAllSearchTeachers = asyncHandler(async (req, res) => {
  try {
    ...
    const transformedTeachersPromises = teachers.map(async (teacher) => {
      ...
      const { groupPaymentId, singlePaymentId } = transformedTeacher;

      // Fetch the TeacherPayment records for both group and single payment
      if (groupPaymentId) {
        groupAmount = await TeacherPayment.findById(groupPaymentId);
      }
      if (singlePaymentId) {
        singleAmount = await TeacherPayment.findById(singlePaymentId);
      }

      // Assign payments directly to the transformedTeacher object
      transformedTeacher.groupAmount = groupAmount ? groupAmount.amount : null;
      transformedTeacher.singleAmount = singleAmount ? singleAmount.amount : null;
    ...
    const favoriteTeachersWithRating = await Promise.all(
      favorite.teacher_ids.map(async (teacher) => {
        ...
        return {
          ...teacher.toObject(),
          averageRating,
          groupAmount: groupPayment ? groupPayment.amount : null,
          singleAmount: singlePayment ? singlePayment.amount : null,
        };
      })
    );
...
const askForDemo = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(user_id);
    const cb_id = user.ConnectyCube_id;
    ...
    if (type === "askDemoids") {
      ...
      // }
      // Add cb_id to the course if not already included
      if (!course.completeAskDemoids.includes(cb_id)) {
        course.completeAskDemoids.push(cb_id);
      }
    }
