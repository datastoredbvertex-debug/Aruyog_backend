...
const getTeacherPaymentStatuses = asyncHandler(async (req, res) => {
      try {
        const { search, page = 1, limit = 10 } = req.query; // Get search, page, and limit from query parameters

        // Query to get users with the role 'teacher'
        const query = { role: "teacher" };

        // Include search in the query if it exists
        if (search) {
          query.$or = [{ full_name: { $regex: search, $options: "i" } }, { email: { $regex: search, $options: "i" } }, { mobile: { $regex: search, $options: "i" } }];
        }
        ...
        // Create promises to fetch transactions and payment status for each user
        const transformedUsersPromises = users.map(async (user) => {
          const teacher_id = user._id;

          // Fetch transactions for the current teacher and calculate total amount
          const transactions = await Transaction.find({ teacher_id });
          const totalAmount = transactions.reduce((sum, transaction) => sum + transaction.amount, 0);

          // Get the latest payment status and all payment statuses for total paid amount
          ...
          // Default remaining amount and payment details
        ...
        res.json({
          ...
          page: parseInt(page),
          limit: parseInt(limit),
          totalRecords: await User.countDocuments(query), // Total number of records
        });
      } catch (error) {
        console.error(error);
        res.status(500).json({
