...
const sendCourseNotification = asyncHandler(async (req, res) => {
  const { course_id, meeting_id, call_id, attended_at, caller_id, call_name, opponents_ids } = req.body;
  ...
  try {
    ...
    course.caller_id = caller_id
    course.call_name = call_name
    course.opponents_ids = opponents_ids || []
    await course.save();

    // Fetch all users subscribed to the course
//     const users = await User.find({
//       _id: { $in: course.userIds },
//       _id: { $in: course.askDemoids },
//     });

    const users = await User.find({
      ConnectyCube_id: { $in: course.opponents_ids }
    });
